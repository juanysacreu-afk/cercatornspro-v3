...
function parse() {
    ...
    for (let line of lines) {
        ...
        // Strict Header Check: only accept the grid pages which have '20 2 6' artifact from pipes
        if (cleanLine.includes('Calendari de serveis') && cleanLine.includes('20 2 6')) {
            parsing = true;
        ...
        for (let t of tokens) {
            ...
            if (/^\d+$/.test(t)) {
                ...
                // Use token history to detect the header sequence `20 2 6` and remove garbage
                if (tokenHistory.length === 3 && tokenHistory[0] === 20 && tokenHistory[1] === 2 && tokenHistory[2] === 6) {
                    flush(daysBuf, codesBuf, currentPair, result); // flush any previous valid data (e.g. from previous page end)
                    daysBuf = [];
                    codesBuf = [];
                    mode = 'DAYS';
                    ignoreUntilOne = true;
