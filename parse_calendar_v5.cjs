...
function parse() {
    ...
    for (let line of lines) {
        ...
        if (cleanLine.includes('Calendari de serveis') && (cleanLine.includes('2026') || cleanLine.includes('20 2 6'))) {
            parsing = true;
            // We don't reset buffers here because we depend on tokenHistory logic to clear garbage from THIS line
        }
        ...
        for (let t of tokens) {
            ...
            if (/^\d+$/.test(t)) {
                ...
                // Track history for 20-2-6 detection
