...
export const OrganitzaView: React.FC = () => {
  ...
  const fetchFullTurnData = async (turnId: string) => {
    ...
    const fullCircs = circulations.map((c: any) => {
        ...
        return { ...detail, ...c, codi, machinistInici, machinistFinal, train: cycleInfo?.train_number };
    }).sort((a: any, b: any) => getFgcMinutes(a.sortida || '00:00') - getFgcMinutes(b.sortida || '00:00'));

    return { ...shift, driver: { nom: assignment?.nom || 'No assignat', nomina: assignment?.empleat_id || '---', phones: driverPhone?.phones || [] }, fullCirculations: fullCircs };
  };
