...
function parse() {
    ...
    for (let line of lines) {
        ...
        if (cleanLine.includes('Calendari de serveis') && (cleanLine.includes('2026') || cleanLine.includes('20 2 6'))) {
            parsing = true;
            console.log('--- Start Parsing Detected ---');
        }
        ...
        if (cleanLine.includes('4. Festes Locals')) {
            ...
            daysBuf = []; codesBuf = [];
            console.log('--- Stop Parsing Detected ---');
            continue;
        }
        ...
        for (let t of tokens) {
            ...
            if (MONTH_MAP.hasOwnProperty(upper)) {
                console.log(`Month detected: ${upper}`);
            ...
            if (/^\d+$/.test(t)) {
                ...
                if (tokenHistory.length === 3 && tokenHistory[0] === 20 && tokenHistory[1] === 2 && tokenHistory[2] === 6) {
                    console.log('Header 20 2 6 detected. Resetting.');
                    flush(daysBuf, codesBuf, currentPair, result);
                    daysBuf = [];
                    codesBuf = [];
                    mode = 'DAYS';
                ...
                if (ignoreUntilOne) {
                    if (val === 1) {
                        console.log('Found 1. Resume parsing.');
                        ignoreUntilOne = false;
                    } else {
                        // console.log(`Ignoring ${val} waiting for 1`);
                        continue;
                    }
                }
                ...
                if (isCode) {
                    ...
                    if (mode === 'CODES') {
                        console.log(`Flush triggered by day ${val}. CodesBuf: ${codesBuf.join(',')}`);
                        flush(daysBuf, codesBuf, currentPair, result);
                        daysBuf = [];
                        codesBuf = [];
                        mode = 'DAYS';
...
function flush(days, codes, pair, result) {
    ...
    for (let k = 0; k < days.length - 1; k++) {
        if (days[k + 1] < days[k] && days[k + 1] === 1) { // Only strict reset to 1
            splitIdx = k + 1;
            break;
        }
    }

    console.log(`Flushing. Days: ${days.length}, Codes: ${codes.length}. StartDay:${days[0]}. Split:${splitIdx}`);
